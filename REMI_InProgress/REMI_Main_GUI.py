...
from RecipePreprocessor import isCookingVerb, parseTechniques, parsefromRecipes, parse_recipe

from REMI_RecipeScraper import validateLink, allRecipeScrape
...
class UserState():
    def __init__(self,
        ...
        self.userIngredients = ingredients #list of ingredients
    ...
    try:
        ...
        if intent == 'u':                            #Im not using this, currently is switches to handle error state for this.
            ...
            if(userIn.lower().find("exit") != -1):
                ...
                pickle.dump(recipeData, open('recipeData.dat', 'wb'))
            ...
            print("New recipe successfully loaded!")
            return "Thanks for giving me that new recipe! I've updated my knowledge based on what I found."
...
def getUserIntent(userstring):
    ...
    startlink = userstring.find("https://www.allrecipes.com/")
    if(startlink != -1):
        linkstring = userstring[startlink:]
        endlink = linkstring.find(' ')
        linkstring = linkstring[:endlink] if endlink != -1 else linkstring
        print("New Link Found:", linkstring, "\nParsing...")
        rawfile = "allrecipes\\" + allRecipeScrape(linkstring) + ".txt"

        userstring = userstring[startlink+len(linkstring):] if linkstring == 0 else (userstring[:startlink] + userstring[startlink + len(linkstring):])
        newRecipe = parse_recipe('{'+ open(rawfile, 'r', encoding='utf8').read()[1:] + '}',nlp=nlp)

        recipeData.update({newRecipe.name:newRecipe})
        userList[currentUserId].recipeCatalog.update({newRecipe.name:newRecipe})
        newIngredients, newMethods = parsefromRecipes({newRecipe.name:newRecipe}, nlp=nlp)
        ingredientData.append(list(set(newIngredients)))
        userList[currentUserId].userIngredients.append(list(set(newIngredients)))
        methodData.append(list(set(newMethods)))
        userList[currentUserId].preferredMethods.append(list(set(newMethods)))

        intent = 'nr'
        workingRecipe = newRecipe
        return intent
